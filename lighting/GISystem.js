...
export class GISystem {
        ...
        for (let iz = 0; iz < rz; iz++) {
            for (let iy = 0; iy < ry; iy++) {
                for (let ix = 0; ix < rx; ix++) {
                    ...
                    const position = new THREE.Vector3(this.minBounds.x + fx * size.x, this.minBounds.y + fy * size.y, this.minBounds.z + fz * size.z);
        ...
        for (const probe of this.probes) {
            ...
            if (lights) {
                for (const light of lights) {
                    if (!light.visible || !(light instanceof THREE.Light))
                        continue;
     ...
     */
    sampleIrradiance(worldPos, normal) {
        const { x: rx, y: ry, z: rz } = this.gridRes;
        const size = this.maxBounds.clone().sub(this.minBounds);
        ...
        const ty = fy - y0;
        const tz = fz - z0;
        const getIdx = (ix, iy, iz) => iz * (rx * ry) + iy * rx + ix;
...
//# sourceMappingURL=GISystem.js.map