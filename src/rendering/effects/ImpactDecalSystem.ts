...
export class ImpactDecalSystem {
  ...
  private readonly baseTex: THREE.Texture | null;
  private readonly bloodTex: THREE.Texture | null;
  private readonly bloodPoolTex: THREE.Texture | null;
  ...
  private makeBulletHoleTexture(): THREE.Texture | null {
  ...
  private makeBloodSplatterTexture(): THREE.Texture | null {
    if (typeof document === 'undefined' || typeof document.createElement !== 'function') return null;
    const size = 128;
    const c = document.createElement('canvas');
    c.width = c.height = size;
  ...
  private makeBloodPoolTexture(): THREE.Texture | null {
    if (typeof document === 'undefined' || typeof document.createElement !== 'function') return null;
    const size = 128;
    const c = document.createElement('canvas');
    c.width = c.height = size;
  ...
  spawnScorch(position: { x: number; y: number; z: number }, radius = 0.5): void {
    const d = this.next();
    if (!d) return;
  ...
  spawnBloodPool(position: { x: number; y: number; z: number }, scale = 1.1): void {
    const d = this.next();
    if (!d) return;
