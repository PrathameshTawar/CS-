...
export class AbilityFXSystem {
  ...
  private readonly smokeClouds: ActiveSmoke[] = [];
  private readonly electricArcs: ActiveElectricArc[] = [];
  private readonly ghosts: ActiveGhost[] = [];
  private flashbangIntensity = 0;
   ...
   */
  spawnSmokeCloud(pos: { x: number; y: number; z: number }, duration = 8.0, radius = 6.0): void {
   ...
   */
  triggerFlashbang(intensity = 1.0): void {
   ...
   */
  spawnElectricArcs(pos: { x: number; y: number; z: number }, radius = 4.5, duration = 1.5): void {
    ...
    for (let i = 1; i <= segs; i++) {
      ...
      points.push(new THREE.Vector3(
        pos.x + Math.cos(angle) * r + (Math.random() - 0.5) * 1.2,
        pos.y + 0.5 + (Math.random() - 0.5) * 1.5,
        pos.z + Math.sin(angle) * r + (Math.random() - 0.5) * 1.2,
      ));
    }
   ...
   */
  spawnGhostTrail(sourceObject: THREE.Object3D, duration = 0.35): void {
  ...
  update(deltaTime: number): void {
    ...
    for (let i = this.electricArcs.length - 1; i >= 0; i--) {
      ...
      if (ea.life >= ea.maxLife) {
        ...
        const posAttr = ea.line.geometry.getAttribute('position') as THREE.BufferAttribute;
        if (posAttr) {
          for (let j = 1; j < posAttr.count; j++) {
            posAttr.setXYZ(
              j,
              ea.pos.x + (Math.random() - 0.5) * ea.radius,
              ea.pos.y + 0.5 + (Math.random() - 0.5) * 1.5,
              ea.pos.z + (Math.random() - 0.5) * ea.radius,
            );
          }
    ...
    for (let i = this.ghosts.length - 1; i >= 0; i--) {
      ...
      if (gh.life >= gh.maxLife) {
        ...
        (gh.mesh.material as THREE.Material).dispose();
        this.ghosts.splice(i, 1);
      } else {
        const alpha = 0.45 * (1 - gh.life / gh.maxLife);
        (gh.mesh.material as THREE.MeshBasicMaterial).opacity = alpha;
      }
    }
  }
