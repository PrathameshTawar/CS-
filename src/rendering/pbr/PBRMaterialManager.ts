 ...
 * The underlying Three.js material is MeshPhysicalMaterial (a superset of
 * MeshStandardMaterial) whenever advanced PBR features are non-zero,
 * and MeshStandardMaterial otherwise for lower draw-call overhead.
 */
interface PBRMaterialInstance {
  material: THREE.MeshStandardMaterial; // may actually be MeshPhysicalMaterial at runtime
 ...
 */
function requiresPhysicalMaterial(params: PBRMaterialParams): boolean {
  return (
    params.clearcoat > 0 ||
    params.sheen > 0 ||
    params.anisotropy > 0 ||
...
export class PBRMaterialManager {
  private readonly materialCache: Map<string, PBRMaterialInstance> = new Map();
  ...
  private generateCacheKey(
    ...
    if (params.baseColor) parts.push(`c_${params.baseColor.getHex()}`);
    if (params.metallic !== undefined) parts.push(`m_${params.metallic.toFixed(3)}`);
    if (params.roughness !== undefined) parts.push(`r_${params.roughness.toFixed(3)}`);
    if (params.alphaTest !== undefined) parts.push(`at_${params.alphaTest.toFixed(3)}`);
    if (params.transparency !== undefined) parts.push(`tr_${params.transparency.toFixed(3)}`);
    if (params.clearcoat !== undefined) parts.push(`cc_${params.clearcoat.toFixed(3)}`);
    if (params.sheen !== undefined) parts.push(`sh_${params.sheen.toFixed(3)}`);
    if (params.anisotropy !== undefined) parts.push(`an_${params.anisotropy.toFixed(3)}`);
    if (params.transmission !== undefined) parts.push(`tx_${params.transmission.toFixed(3)}`);

    // Add texture IDs (using texture UUID or name)
    const textureKeys: (keyof PBRTextureSet)[] = [
  ...
  getMaterial(
    ...
    // Create new material
    const mergedParams: PBRMaterialParams = {
      ...DEFAULT_PBR_PARAMS,
      ...params,
    };
  ...
  updateMaterial(
    ...
    if (params.baseColor !== undefined) material.color.copy(params.baseColor);
    ...
    if (params.emissiveColor !== undefined) material.emissive.copy(params.emissiveColor);
    ...
    if (params.occlusionStrength !== undefined) material.aoMapIntensity = params.occlusionStrength;
  ...
  applyTextures(
    ...
    if (textures.albedoMap) material.map = textures.albedoMap;
    if (textures.normalMap) material.normalMap = textures.normalMap;
    if (textures.metallicRoughnessMap) {
      material.metalnessMap = textures.metallicRoughnessMap;
      material.roughnessMap = textures.metallicRoughnessMap;
    }
    if (textures.occlusionMap) material.aoMap = textures.occlusionMap;
