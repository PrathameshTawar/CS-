...
export class EnemySoldierRig {
  ...
  constructor(def: EnemyClassDef, scale = 1) {
  ...
  private makeLeg(mat: THREE.Material, h: number): THREE.Group {
  ...
  private makeArm(mat: THREE.Material, h: number, left: boolean): THREE.Group {
    const g = new THREE.Group();
  ...
  private buildWeapon(def: EnemyClassDef, dark: THREE.Material, h: number): void {
