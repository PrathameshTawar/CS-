...
export class HUD {
    ...
    disposers = [];
    // Minimap state
    minimapBounds = { width: 90, depth: 90, offsetX: 0, offsetZ: 0 };
    minimapEnemies = [];
    playerWorldPos = new THREE.Vector3();
    playerYaw = 0;
    // Compass tick-tape layout (cached refs avoid per-frame querySelector/layout reads)
    compassPxPerDeg = 1.5;
    compassCopyWidth = 360 * 1.5;
    compassStripEl = null;
    compassTextEl = null;
    compassPanelWidth = 0;
    ...
    constructor(bus, camera, config) {
      ...
      <div style="display:flex;flex-direction:column;gap:6px;font-family:'Segoe UI',system-ui,sans-serif;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,0.9);">
        ...
        this.healthFill = this.healthEl.querySelector('#hpFill');
        this.armorFill = this.healthEl.querySelector('#armorFill');
     ...
     */
    static ensureGlassStyles() {
        if (document.getElementById('hud-glass-style'))
            return;
        const st = document.createElement('style');
     ...
     */
    buildCompassStrip() {
        const strip = this.compassEl.querySelector('#hudCompassStrip');
        this.compassStripEl = strip;
        this.compassTextEl = this.compassEl.querySelector('#hudCompassText');
        if (!strip)
            return;
    ...
    registerEvents() {
        const bus = this.bus;
        this.disposers.push(bus.on(GAME_EVENTS.AMMO, (e) => this.onAmmo(e)), bus.on(GAME_EVENTS.HEALTH, (e) => this.onHealth(e)), bus.on(GAME_EVENTS.HIT_MARKER, (e) => this.onHitMarker(e)), bus.on(GAME_EVENTS.KILL_FEED, (e) => this.onKillFeed(e)), bus.on(GAME_EVENTS.DAMAGE, (e) => this.onDamage(e)), bus.on(GAME_EVENTS.KILL, () => this.onKill()), bus.on(GAME_EVENTS.ABILITY, (e) => this.onAbility(e)), bus.on(GAME_EVENTS.OBJECTIVE, (e) => this.onObjective(e)), bus.on(GAME_EVENTS.SQUAD, (e) => this.onSquad(e)), bus.on(GAME_EVENTS.DAMAGE, (e) => {
            if (e.target === 'player') {
                this.flashDamage();
    ...
    onAmmo(e) {
        const mag = this.ammoEl.querySelector('#ammoMag');
        const res = this.ammoEl.querySelector('#ammoRes');
        if (mag)
            mag.textContent = String(e.magazine);
        if (res)
            res.textContent = `/ ${e.reserve}`;
        if (e.reloading) {
    ...
    onHealth(e) {
        const hpVal = this.healthEl.querySelector('#hpVal');
        const armorVal = this.healthEl.querySelector('#armorVal');
        if (hpVal)
            hpVal.textContent = String(Math.ceil(e.health));
        if (armorVal)
            armorVal.textContent = String(Math.ceil(e.armor));
        ...
        if (e.health < e.maxHealth * 0.25) {
            ...
            this.vignette.animate([{ opacity: 0.4 }, { opacity: 0.7 }, { opacity: 0.4 }], { duration: 1000, iterations: Infinity });
        }
        else {
    ...
    onDamage(e) {
        if (e.target !== 'enemy' || !e.worldPosition)
            return;
    ...
    onAbility(e) {
        const bar = this.abilityBar.querySelector(`[data-ability="${e.id}"]`);
        if (!bar)
            return;
    ...
    onObjective(e) {
        ...
        if (e.isWave && e.progress) {
            ...
            void this.waveCounterEl.offsetWidth;
    ...
    spawnDamageNumber(world, amount, critical) {
    ...
    showDirectionalDamage(e) {
        ...
        let t = Infinity;
        if (Math.abs(s) > 1e-4)
            t = Math.min(t, (w / 2) * (1 - margin) / Math.abs(s));
        if (Math.abs(c) > 1e-4)
            t = Math.min(t, (h / 2) * (1 - margin) / Math.abs(c));
        const ex = cx + s * t;
        const ey = cy - c * t;
    ...
    update(deltaTime) {
        ...
        if (this.compassTextEl)
            this.compassTextEl.textContent = `${dir} ${Math.round(bearing)}°`;
        if (this.compassStripEl) {
            // Cache the panel width after first layout (it is static) to avoid reflow churn.
            if (this.compassPanelWidth === 0)
                this.compassPanelWidth = this.compassEl.clientWidth || 340;
     ...
     */
    setMinimapData(bounds, enemies, playerPos, playerYaw) {
     ...
     */
    updateSkullerBadge(skullCount, rankTitle, equippedSkinName) {
        ...
        if (countEl)
            countEl.textContent = `💀 ${skullCount} SKULLS (${this.escapeHtml(rankTitle)})`;
        if (skinEl)
            skinEl.textContent = this.escapeHtml(equippedSkinName.toUpperCase());
    }
    /**
     * Show dramatic animated Skuller emoji overlay on one-tap headshot kill.
     */
    showSkullerHeadshotOverlay(skullCount, rankTitle) {
     ...
     */
    showSkullerSkinsModal(rewards, onEquip) {
      ...
      `;
            ...
            modal.querySelectorAll('.equipSkinBtn').forEach((btn) => {
                btn.addEventListener('click', (e) => {
                    const sid = e.currentTarget.getAttribute('data-skin-id');
