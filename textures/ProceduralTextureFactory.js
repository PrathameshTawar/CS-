...
function valueNoise(px, py, seed) {
...
function paintGrunge(map, bump, rough, base, seed, scale = 6, grungeAmp = 0.14, bumpAmp = 60, roughLuma = 0.75) {
...
function paintCracks(ctx, seed, count, color) {
...
function paintWood(ctx, seed) {
    ...
    for (let p = 0; p < 5; p++) {
        ...
        for (let g = 0; g < 4; g++) {
            ...
            ctx.quadraticCurveTo(p * plankW + plankW / 2, gy + (hash01(p, g, seed + 3) - 0.5) * 24, (p + 1) * plankW - 4, gy);
            ctx.stroke();
        }
    }
}
/** Draw metal panel seams + rivets. */
function paintMetal(ctx, _seed) {
    const cols = 4;
    const rows = 4;
...
function paintSpeckle(ctx, seed, color, count) {
    ctx.fillStyle = color;
...
function paintCobblestone(ctx, seed) {
    const cols = 8;
    const rows = 12;
    const cw = TEX_SIZE / cols;
    const ch = TEX_SIZE / rows;
    for (let cy = 0; cy < rows; cy++) {
        ...
        for (let cx = -1; cx < cols + 1; cx++) {
            ...
            if (h < 0.3)
                ctx.fillStyle = 'rgba(255, 235, 200, 0.34)';
            else if (h < 0.6)
                ctx.fillStyle = 'rgba(184, 150, 108, 0.36)';
            else
                ctx.fillStyle = 'rgba(120, 96, 70, 0.4)';
...
function paintPeelingStucco(ctx, seed) {
...
function paintGraffitiStencil(ctx, seed) {
    const h = hash01(99, 88, seed);
    if (h > 0.45)
        return; // 45% chance per wall tile
    ctx.save();
...
function makeMaterial(_kind, baseColor, seed, roughness, metalness, paint, tile, transparent = false) {
 ...
 */
export function createGroundMaterial(biome, baseColor, seed = 1337) {
    switch (biome) {
        case Biome.City:
...
export function createBlockMaterial(material, baseColor, seed = 1337) {
    switch (material) {
        case 'concrete':
...
export function disposeProceduralMaterial(mat) {
    if (!mat)
        return;
    const textures = mat.userData?.proceduralTextures;
    if (textures) {
...
//# sourceMappingURL=ProceduralTextureFactory.js.map