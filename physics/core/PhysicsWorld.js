...
function insertIntoGrid(grid, block) {
...
function removeFromGrid(grid, block) {
    const [x0, y0, z0, x1, y1, z1] = blockCells(block);
    for (let gx = x0; gx <= x1; gx++) {
        for (let gy = y0; gy <= y1; gy++) {
            for (let gz = z0; gz <= z1; gz++) {
...
export class PhysicsWorld {
    blocks = [];
    bounds = { width: 90, depth: 90, height: 20 };
     ...
     */
    onBlockDamaged;
    constructor(blocks = [], bounds) {
        if (bounds)
            this.bounds = bounds;
        this.setWorld(blocks, bounds ?? this.bounds);
    }
    setWorld(blocks, bounds) {
        this.blocks = blocks;
        this.bounds = bounds;
        this.grid.clear();
     ...
     */
    _rayGridCandidates(origin, dir, maxDist) {
