...
export class CreatorMode {
     ...
     */
    parseAndExecuteCommand(promptText) {
        ...
        if (/add|spawn|create/i.test(lower)) {
            if (/sniper|scout|heavy|medic|engineer/i.test(lower)) {
                ...
                const classId = (match ? match[0].toLowerCase() : 'scout');
                mutation = {
                    id,
                    timestamp,
                    rawCommand,
        ...
        // 3. Time of day mutations ("make it night", "make it dusk", "daytime")
            ...
            let timeOfDay = 'day';
            if (/night|dark/i.test(lower))
                timeOfDay = 'night';
            else if (/dusk/i.test(lower))
                timeOfDay = 'dusk';
            mutation = {
                id,
                timestamp,
        ...
        // 4. Weather mutations ("storm", "rain", "snow", "fog", "clear")
            ...
            let weather = 'clear';
            if (/storm|rain/i.test(lower))
                weather = 'storm';
            else if (/snow/i.test(lower))
                weather = 'snow';
            else if (/fog/i.test(lower))
                weather = 'fog';
            else if (/ash/i.test(lower))
                weather = 'ash';
            mutation = {
                id,
                timestamp,
        ...
        // 5. Difficulty ("make it harder", "set difficulty hard")
            ...
            if (/hard/i.test(lower))
                difficulty = 'hard';
            else if (/easy/i.test(lower))
                difficulty = 'easy';
...
//# sourceMappingURL=CreatorMode.js.map