...
import { MapGenerator } from '../../gameplay/maps/MapGenerator';
const TIME_PRESETS = {
    day: {
        sunMul: 1.0,
        hemiMul: 1.0,
...
export class WorldMutator {
    ...
    weather = 'clear';
    timeOfDay = 'day';
    constructor(targets, biome, initial) {
        this.targets = targets;
        this.biome = biome;
        if (initial?.weather)
            this.weather = initial.weather;
        if (initial?.timeOfDay)
            this.timeOfDay = initial.timeOfDay;
    ...
    apply(mutation) {
        if (mutation.weather)
            this.weather = mutation.weather;
        if (mutation.timeOfDay)
            this.timeOfDay = mutation.timeOfDay;
        this.write(mutation);
    }
    ...
    write(mutation) {
        ...
        if (sky) {
            const cloudCover = this.weather === 'storm' ? 0.95 :
...
//# sourceMappingURL=WorldMutator.js.map