import { BALANCE_ENVELOPE, validateBalancePayload } from '../../engine/content/ContentSchemas';
import { ENEMY_CLASSES } from '../../ai/classes/EnemyClasses';
export class BalanceAgent {
    currentMultiplierSet = null;
    fromMultiplierSet = null;
    targetMultiplierSet = null;
    currentDifficulty = 'normal';
    transitioning = false;
    transitionTime = 0;
    transitionDuration = 10; // ≥10 s ramp (R31.4)
    constructor() {
        this.currentMultiplierSet = this.generateDefaultMultipliers('normal');
    }
    generateDefaultMultipliers(difficulty) {
        ...
        const classIds = Object.keys(ENEMY_CLASSES);
    ...
    setDifficulty(difficulty, immediate = true) {
        if (this.currentDifficulty === difficulty && !this.transitioning)
            return;
    ...
    applyBalancePayload(payload) {
        ...
        if (this.currentMultiplierSet[payload.enemyClass]) {
            this.currentMultiplierSet[payload.enemyClass] = { ...payload };
            return true;
        }
        return false;
    }
    getMultiplier(classId) {
        if (!this.currentMultiplierSet)
            return null;
        return this.currentMultiplierSet[classId] || null;
    }
}
//# sourceMappingURL=BalanceAgent.js.map