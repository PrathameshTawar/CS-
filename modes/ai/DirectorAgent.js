 ...
 */
export const DEFAULT_DIRECTOR_RULES = [
    {
        id: 'low-health-medkit',
        cooldown: 30,
        probability: 0.85,
        condition: (f) => f.health > 0 && f.maxHealth > 0 && f.health / f.maxHealth < LOW_HEALTH_RATIO && f.lowHealthSeconds >= LOW_HEALTH_SECONDS,
    ...
    },
    {
        ...
        cooldown: 20,
        probability: 1,
        condition: (f, ctx) => f.killStreak >= KILL_STREAK_RAMP && ctx.difficulty !== 'hard' && ctx.difficultyCeiling === 'hard',
    ...
    },
    {
        ...
        make: () => ({ kind: 'spawn_enemies', count: 2, classes: ['scout', 'heavy'], urgency: 0.6 }),
    },
];
/**
...
export class DirectorAgent {
    ...
    tick() {
        ...
        if (this.health > 0 && ratio < LOW_HEALTH_RATIO)
            this.lowHealthSeconds += 1;
        else
            this.lowHealthSeconds = 0;
        const frame = this.buildFrame(now, moving, firing);
        const commands = this.evaluate(frame);
        for (const cmd of commands) {
            if (cmd.kind === 'spawn_enemies')
                this.spawnedEnemies += cmd.count;
    ...
    evaluate(frame) {
        ...
        for (const rule of this.rules) {
            ...
            if (now - last < rule.cooldown * 1000)
                continue;
            if (!rule.condition(frame, ctx))
                continue;
            if (this.options.random() >= rule.probability)
                continue;
    ...
    subscribe() {
        ...
        }), this.bus.on(GAME_EVENTS.WEAPON_FIRED, (e) => {
            if (e.sourceId === -1) {
                this.shotsFired++;
                this.lastFireAt = this.options.now();
            }
        }), this.bus.on(GAME_EVENTS.HIT_MARKER, () => {
            this.shotsHit++;
        }), this.bus.on(GAME_EVENTS.SOUND, (e) => {
            if (e.type === 'footstep' && e.sourceId === -1)
                this.lastFootstepAt = this.options.now();
        }), this.bus.on(GAME_EVENTS.OBJECTIVE, (e) => {
            if (e.progress) {
    ...
    buildFrame(now = this.options.now(), moving = false, firing = false) {
        return {
            timestamp: now,
            health: this.health,
...
//# sourceMappingURL=DirectorAgent.js.map